...
Hooks.on('renderCombatTracker', async (
    ...
    for (const combatant of tracker.viewed.combatants){
        if(advancedCombatsMap[tracker.viewed.id].combatantIdToCombatantActionsMap.get(combatant.id) == undefined){
...
Hooks.on("updateCombatant", async (
    ...
    activeCombat.combatantIdToCombatantActionsMap.get(combatant?.id!)?.pullFlagInformation();
});